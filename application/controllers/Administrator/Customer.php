...
class Customer extends CI_Controller
{
    ...
    public function addCustomer()
    {
        ...
        try {
            ...
            $checkExistCustomerName = $this->db->query("SELECT * FROM `tbl_customer` WHERE `Customer_Name` = ?", $customerObj->Customer_Name);
            if ($checkExistCustomerName->num_rows() > 0) {
                $res = ['success' => false, 'message' => 'Duplicate Customer Name'];
                echo json_encode($res);
                exit;
            }
    ...
    public function updateCustomer()
    {
        ...
        try {
            ...
            $checkExistCustomerName = $this->db->query("SELECT * FROM `tbl_customer` WHERE `Customer_Name` = ? and Customer_SlNo != ?", [$customerObj->Customer_Name, $customerObj->Customer_SlNo]);
            if ($checkExistCustomerName->num_rows() > 0) {
                $res = ['success' => false, 'message' => 'Duplicate Customer Name'];
                echo json_encode($res);
                exit;
