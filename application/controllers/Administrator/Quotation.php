...
class Quotation extends CI_Controller
{
    ...
    public function addQuotation()
    {
        ...
        try {
            ...
            $quotation = array(
                ...
                'Ref_Number'                     => $data->quotation->Ref_Number,
                'currency_name'                  => $data->quotation->currency_name,
                'terms_condition'                => $data->quotation->terms_condition,
    ...
    public function getQuotations()
    {
        ...
        $res['quotations'] = $this->db->query("SELECT qm.*,c.*
    ...
    public function sendQuotationByMail()
    {
        $res = ['success' => false, 'message' => ''];
        try {
            $data = json_decode($this->input->raw_input_stream);
            ...
            $quotation['quotation'] = $quotations->row();
            $quotation['quotationDetails'] = $quotationDetails->result();

            // $this->load->library('pdf');
            // $view = $this->load->view('Administrator/quotation/mail_quotation', $quotation, true);
            // $this->pdf->createPDF($view, 'Quotation-' . $data->quotation, false);
            ...
            // $pdfFilePath = './uploads/Quotation-' . $data->quotation . '.pdf';
            // $this->email->from($from_email, $this->session->userdata("FullName"));
            // $this->email->to($to_email);
            // $this->email->subject('Quotation');
            // $this->email->message('Dear Customer, Please see attachment Quotation!');
            // $this->email->attach($pdfFilePath);
            // if ($this->email->send()) {
            //     unlink($pdfFilePath);
            //     $res = ['success' => true, 'message' => 'Mail Send!'];
            // } else {
            //     $res = ['success' => true, 'message' => 'Mail Send Failed!'];
            // }
            $res = ['success' => true, 'message' => 'Mail Send Success!'];
        } catch (\Exception $e) {
