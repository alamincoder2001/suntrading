...
class Model_Table extends CI_Model
{
    ...
    public function generateSalesInvoice()
    {
        ...
        $invoice = date('ymd') . $branchNo . "000001";
        $year = date('y');
    ...
    public function getBankTransactionSummary($accountId = null, $date = null)
    {
        $bankTransactionSummary = $this->db->query("
            select 
                ...
                (
                    select (received_sales + ba.initial_balance + total_deposit + total_received_from_customer + total_received_from_supplier) - (paid_purchase+ total_withdraw + total_paid_to_customer + total_paid_to_supplier)
                ) as balance
